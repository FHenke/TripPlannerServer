...
private int successConnections = 0;
	...
	private LinkedBlockingQueue<Connection> getOnlyCheepestConnectionFromResult(ResultSet results){
		...
		Place origin;
		try {
			origin = Query.setAirportinformationFromDatabase(new Place(segments[0].getOrigin(), segments[0].getOrigin(), Place.AIRPORT));
			Place destination = Query.setAirportinformationFromDatabase(new Place(segments[segments.length - 1].getDestination(), segments[segments.length - 1].getDestination(), Place.AIRPORT));
